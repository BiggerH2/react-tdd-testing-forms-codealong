...
import '@testing-library/jest-dom/extend-expect'; // Importing extended matchers
import App from "../App";
...
test("size select element initially displays 'Small'", () => {
  ...
  expect(selectSize).toHaveValue("small"); // Updated toHaveDisplayValue to toHaveValue
});
...
test("select Size dropdown displays the user's selected value", () => {
  ...
  expect(selectSize).toHaveValue("medium"); // Updated toHaveDisplayValue to toHaveValue

  userEvent.selectOptions(selectSize, "large");

  expect(selectSize).toHaveValue("large"); // Updated toHaveDisplayValue to toHaveValue
});
